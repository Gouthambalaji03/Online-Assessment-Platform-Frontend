...
const MyExams = () => {
  ...
  const getExamStatus = (exam) => {
    ...
    if (exam.status === 'completed') return { status: 'Completed', badge: 'badge-neutral' };
    if (examDate < now) return { status: 'Expired', badge: 'badge-danger' };
    if (exam.status === 'active') return { status: 'Active', badge: 'badge-success' };
    return { status: 'Upcoming', badge: 'badge-info' };
  };
  ...
  return (
    <Layout>
      <div className="animate-fadeIn">
        <div className="mb-8">
          <h1 className="text-[32px] font-bold text-[#1E293B] mb-2">My Exams</h1>
          <p className="text-[#64748B]">View and manage your enrolled assessments</p>
        </div>
        ...
        {exams.length > 0 ? (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="text-xl font-semibold text-[#1E293B]">{exam.title}</h3>
                          <span className={`badge ${statusInfo.badge}`}>
                            {statusInfo.status}
                          </span>
                        </div>
                        <p className="text-[#64748B] text-sm mb-3">{exam.description || 'No description'}</p>
          ...
          <div className="glass-card p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-[#1E293B] mb-2">No Enrolled Exams</h3>
            <p className="text-[#64748B] mb-4">You haven't enrolled in any exams yet</p>
